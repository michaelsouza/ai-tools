where $H(X)$ is the Hamilton's expression for the trace Harnack inequality (with $t = -\tau$). Hence,
 ...
 $$ \bar{\tau}^{\frac{3}{2}}(R+|X|^{2})(\bar{\tau})=-K+\frac{1}{2}L(q,\bar{\tau}), $$ 
...
so, if  $ Y(0) = 0 $ then
 ...
 $$ \begin{align*}\int_{0}^{\tau}\sqrt{\tau}(\nabla_{Y}\nabla_{Y}R+2<R(Y,X),Y,X>+2|\nabla_{X}Y|^{2}\\+2\nabla_{X}\mathrm{Ric}(Y,Y)-4\nabla_{Y}\mathrm{Ric}(Y,X))d\tau,\end{align*} $$ 
