...
$$ \begin{aligned} \delta_Y^2(\mathcal{L}) &= \int_0^{\bar{\tau}} \sqrt{\bar{\tau}}(Y \cdot Y \cdot R + 2 \langle \nabla_Y \nabla_Y X, X \rangle + 2|\nabla_Y X|^2) d\tau \\ &= \int_0^{\bar{\tau}} \sqrt{\bar{\tau}}(Y \cdot Y \cdot R + 2 \langle \nabla_X \nabla_Y Y, X \rangle + 2 \langle R(Y, X), Y, X \rangle + 2|\nabla_X Y|^2) d\tau \end{aligned} $$
